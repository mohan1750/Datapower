...
public class TCPConnectionTest {
	...
	public String testTcp(String ping,String ports,String host,String port)
	{
	    ...
	    SOAPMessage soapResponse = soapConnection.call(createSOAPRequest(ping,ports),url);
	            
	            NodeList nl=soapResponse.getSOAPPart().getEnvelope().getBody().getFirstChild().getChildNodes();
	            System.out.println(nl.item(0).getTextContent());
	            String out;
	            if(nl.item(1).hasChildNodes())
	            {
	            	out=nl.item(1).getFirstChild().getFirstChild().getTextContent();
	            }
	            else
	            {
	            out= nl.item(1).getTextContent();
	            }
	...
	private static SOAPMessage createSOAPRequest(String ping,String port) throws Exception {
        ...
        SOAPElement soapBodyElem2=soapBodyElem1.addChildElement("TCPConnectionTest");
        SOAPElement soapBodyElem3=soapBodyElem2.addChildElement("RemoteHost");
        soapBodyElem3.addTextNode(ping);
        SOAPElement soapBodyElem4=soapBodyElem2.addChildElement("RemotePort");
        soapBodyElem4.addTextNode(port);
