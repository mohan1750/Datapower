...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="/">
		...
		<xsl:variable name="errorStylesheet" select="string($errorConfig/errorHandler/@stylesheet)"/>
		<xsl:message>errorStylesheet:<xsl:value-of select="$errorStylesheet"/> </xsl:message>
               
                <dp:set-variable name="'var://context/ANZ/ErrStyleSheet'" value="$errorStylesheet"/>
				...
				<xsl:variable name="statusBlock">
					<status>
						<xsl:choose>
							...
							<xsl:when test="$errorCode='0x01130006' or $errorCode='0x01130007' or $errorCode='0x01130008' or $errorCode='0x0003000a' or  $errorCode='0x0113001e' or $errorCode='0x0113001c' ">
