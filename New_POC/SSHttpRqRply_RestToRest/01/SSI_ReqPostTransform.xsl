...
<xsl:copy-of select="$rtInfoFile"/>
	...
	<xsl:template name="getRoutingUri">
		...
		<xsl:param name="inpayload"/>
                <xsl:message dp:priority="error">Target Config sent was: <xsl:copy-of select="$tarConfig/TargetConfig/EndpointConfig/uri"/></xsl:message>
		<xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="normalize-space($ssiUriHandler)!=''">
						<xsl:variable name="uriHandlerFile" select="document($ssiUriHandler)"/>
                                                <xsl:message dp:priority="error">The specified SSI Uri Handler 
<xsl:copy-of select="$uriHandlerFile"/>
</xsl:message>
						<xsl:choose>
...
<xsl:value-of select="$ssiUriHandler"/>
 doesn't exist</xsl:message>
			...
			<xsl:otherwise>
                                <xsl:message dp:priority="error">Specified URI is: <xsl:value-of select="$targetConfig/TargetConfig/EndpointConfig/uri"/></xsl:message>
