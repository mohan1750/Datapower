...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dp="http://www.datapower.com/extensions" xmlns:dpconfig="http://www.datapower.com/param/config" xmlns:str="http://exslt.org/strings" xmlns:regexp="http://exslt.org/regular-expressions" xmlns:json="http://www.ibm.com/xmlns/prod/2009/jsonx" extension-element-prefixes="dp" exclude-result-prefixes="dp dpconfig str json">
	...
	<xsl:template match="/">
		...
		<xsl:choose>
			<xsl:when test="/json:object">
				<dp:set-variable name="'var://context/TC/TransformXSL'" value="'store:///jsonx2json.xsl'"/>
			</xsl:when>
			<xsl:otherwise>
				<dp:set-variable name="'var://context/TC/TransformXSL'" value="'store:///identity.xsl'"/>
			</xsl:otherwise>
		</xsl:choose>
		<!-- Setting content type header if not present -->
		<xsl:choose>
